...
def main():
    ...
    parser.add_argument("--test_set",
                        default="dev",
                        choices=["train", "dev", "test", "simple_test", "complex_test", "small_test"],
    ...
    model = BertForSequenceClassification.from_pretrained(load_dir, cache_dir=cache_dir, num_labels=Constants.act_len)
        ...
        for epoch in range(int(args.num_train_epochs)):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                if (step + 1) % args.period == 0:
                    ...
                    precision, recall, F1 = evaluate(args, model, device, processor, label_list, num_labels, tokenizer,
                                                     output_mode)

                    if F1 > best_F1:
                        output_dir = os.path.join(args.output_dir,
                                                  'pre_{}_recall_{}_F1_{}'.format(precision, recall, F1))
    ...
    if args.do_eval:
        ...
        if args.load_dir is not None:
            load_step = print(os.path.split(args.load_dir)[1].replace('save_step_', ''))
